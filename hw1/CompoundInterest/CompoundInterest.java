...
public class CompoundInterest {
    ...
    {
        int numYearsvar;
        numYearsvar = numYears(targetYear);

        int counter = 0;

        double currentValue = presentValue;
        double rates = rate/100 + 1;

        while (counter < numYearsvar)
        {
            currentValue = currentValue * rates;
            counter += 1;
        }
    ...
    {
        double futureValue = futureValue(presentValue, rate, targetYear);

        int counter = 0;
        double inflation = (100-inflationRate) / 100;
        int numYearsVar = numYears(targetYear);

        while (counter < numYearsVar)
        {
            futureValue = futureValue * inflation;
            counter += 1;
        }
    ...
    static double totalSavings(double perYear, int targetYear, double rate)
    {
        ...
        while (count <= numYears(targetYear))
        {
            currentValue += perYear * Math.pow(interestrate, count);
            count += 1;
        }
    ...
    {
        double currentValue = (totalSavings(perYear, targetYear, rate));

        return currentValue*Math.pow(1-inflationRate/100, numYears(targetYear));
    }
